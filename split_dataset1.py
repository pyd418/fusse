...
if __name__ == '__main__':
    ...
    # 随机生成list
    Pt_list = []
    while len(Pt_list) < 50:
        Pts = [np.random.randint(0, 430)]
        fact_dic = get_fact_dic(facts, Pts)
        for pt in fact_dic.keys():
            ...
            # Save tests in file.
            if not os.path.exists("./benchmarks/" + BENCHMARK + '/' + str(pt) + "/test"):
                os.makedirs("./benchmarks/" + BENCHMARK + '/' + str(pt) + "/test")
                os.makedirs("./benchmarks/" + BENCHMARK + '/' + str(pt) + "/train")
                print("Create file!")
