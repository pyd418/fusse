...
class TEXT_CNN(object):
    def __init__(self, config, content_len, vocab2id, is_training=True, desciption_data=None, lengths=None):
        ...
        with tf.name_scope("embedding") as embedding:
            ...
            self.parameter_lists = {"ent_embeddings": self.ent_embeddings, \
                                    ...
                                    "ent_cnn_embedding": self.ent_cnn_embeddings}
        ...
        with tf.name_scope("loss") as loss:
            ...
            # config_bert = BertConfig.from_pretrained("./text_emb/data/bert_config.json")
            # model = BertModel(config_bert, True, self.pos_h_words)
            # p_h_conv_res = self.mlp(model.outputs[0])
            ...
            # model = BertModel(config_bert, True, self.pos_t_words)
            # p_t_conv_res = self.mlp(model.outputs[0])
            ...
            # model = BertModel(config_bert, True, self.neg_h_words)
            # n_h_conv_res = self.mlp(model.outputs[0])
            ...
            # model = BertModel(config_bert, True, self.neg_t_words)
            # n_t_conv_res = self.mlp(model.outputs[0])
    ...
    def mlp(self, input):
        hidden_layer = tf.contrib.layers.fully_connected(input, 128,
                                                         weights_initializer=tf.contrib.layers.xavier_initializer(
                                                             seed=100))
        logits_flat = tf.contrib.layers.linear(hidden_layer, self.embedding_size,
                                               weights_initializer=tf.contrib.layers.xavier_initializer(seed=102))
        logits = tf.reshape(logits_flat, [self.batch_size, self.embedding_size])    # reshape to be (batch_size*4)
        return logits
    ...
    def cbow(self, ent_words_vec, content_len):

        content_len = tf.reshape(content_len, [-1, 1])
        conv_outputs = tf.truediv(tf.reduce_sum(ent_words_vec, 1), tf.cast(content_len, tf.float32), name="outputs")
    ...
    def calc_loss_text_cnn(self, h, r, t, d_h, d_t):
        ...
        return abs(d_h + r - d_t)

    def calc_loss_tre(self, h, r, t):

        return abs(h + r - t)

    def init_word_embedding(self):      # Bert
        ...
        with open(self.pretrain_word_embedding_path, 'r', encoding='UTF-8') as f:
        ...
        # with open('./text_emb/data/vocab.txt', 'w') as f:
        #     for vocab in self.vocab2id:
        #         f.write(vocab+"\n")
        #     f.close()
        #
        # config = BertConfig.from_pretrained('./text_emb/data/bert_config.json')
        # tokenizer = BertTokenizer.from_pretrained('./text_emb/data/vocab.txt', do_lower_case=True)
        #
        ...
        #                    )
        # model = BertModel(config, True, encoded_dict)
        #
        # last_hidden_states_2 = model.outputs
